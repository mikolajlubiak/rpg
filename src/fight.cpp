...
namespace rpg
{
    ...
    void Fight::hit(std::shared_ptr<Player> who, std::shared_ptr<Player> whom, std::uint64_t attack_damage, BodyPart body_part)
    {
    ...
    void Fight::loop()
    {
        ...
        while (m_pEnemy->m_Health > 0 && m_pPlayer->m_Health > 0)
        {
            ...
            if (players_turn)
            {
                ...
                char input;
                std::cout << "What do you want to do? [a - Attack, h - Heal yourself (50%), d - Additional damage (50%)]: ";
                std::cin >> input;

                if (input == 'a')
                {
                    std::cout << "Where do you want to hit your enemy? [b - Boots (50%), c - Chest (90%), h - Head (20%)]: ";
                    std::cin >> input;
                    switch (input)
                    {
                    case 'b':
